...
describe Task do
  ...
  it ".duration_from(tasks, date) returns the duration of the given tasks from the given date" do
    task1 = Fabricate :task
    task1.sessions << Fabricate(:session, :task => task1, :start => DateTime.current.advance(:days => -2), :finish => DateTime.current.advance(:days => -2, :hours => +10)) # 10 hours 2 days ago

    task2 = Fabricate :task
    task2.sessions << Fabricate(:session, :task => task2, :start => DateTime.current.advance(:days => -4), :finish => DateTime.current.advance(:days => -4, :hours => +2)) # 2 hours 4 days ago
    task2.sessions << Fabricate(:session, :task => task2, :start => DateTime.current.advance(:days => -2), :finish => DateTime.current.advance(:days => -2, :hours => +4)) # 4 hours 2 days ago

    task3 = Fabricate :task
    task3.sessions << Fabricate(:session, :task => task3, :start => DateTime.current, :finish => DateTime.current.advance(:hours => +2)) # 2 hours today

    Task.duration_from([task2, task3], Date.today.advance(:days => -2)).should == 6.0
  end
  ...
  it ".duration_on_date(tasks, date) returns the duration of the given tasks on the given date" do
    task1 = Fabricate :task
    task1.sessions << Fabricate(:session, :task => task1, :start => DateTime.current, :finish => DateTime.current.advance(:hours => +10)) # 10 hours today

    task2 = Fabricate :task
    task2.sessions << Fabricate(:session, :task => task2, :start => DateTime.current.advance(:days => -4), :finish => DateTime.current.advance(:days => -4, :hours => +2)) # 2 hours 4 days ago
    task2.sessions << Fabricate(:session, :task => task2, :start => DateTime.current, :finish => DateTime.current.advance(:hours => +4)) # 4 hours today

    task3 = Fabricate :task
    task3.sessions << Fabricate(:session, :task => task3, :start => DateTime.current, :finish => DateTime.current.advance(:hours => +2)) # 2 hours today

    Task.duration_on_date([task2, task3], Date.today).should == 6.0
  end
